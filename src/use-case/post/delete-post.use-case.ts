...
export class DeletePostUseCase {
  ...
  async execute(id: string, requesterId: string): Promise<void> {
    ...
    await Promise.all([
      post.categoryId ? this.categoryRepo.decrementPostCount(post.categoryId) : Promise.resolve(),
      post.tagIds?.length ? this.tagRepo.decrementPostCount(post.tagIds) : Promise.resolve(),
