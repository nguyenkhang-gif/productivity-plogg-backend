...
export class UploadLocalController {
  ...
  async listFiles(
    ...
    return this.uploadLocal.listFiles(req.user.userId, 'upload', +page, +limit);
  }
  ...
  async listIcons(
    ...
    return this.uploadLocal.listFiles(req.user.userId, 'icons', +page, +limit);
  }

  @Delete('files/:fileName')
  @HttpCode(HttpStatus.NO_CONTENT)
  async deleteFile(@Param('fileName') fileName: string, @Req() req) {
    await this.uploadLocal.deleteFile(fileName, req.user.userId, 'upload');
  }

  @Delete('icons/:fileName')
  @HttpCode(HttpStatus.NO_CONTENT)
  async deleteIcon(@Param('fileName') fileName: string, @Req() req) {
    await this.uploadLocal.deleteFile(fileName, req.user.userId, 'icons');
  }
}
