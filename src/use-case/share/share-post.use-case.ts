...
export class SharePostUseCase {
  ...
  async execute(originalPostId: string, userId: string, caption: string): Promise<SharePostResult> {
    ...
    if (original.type === 'REPOST') throw new BadRequestException('Cannot share a repost');

    const existing = await this.postRepo.findRepostByUser(originalPostId, userId);
